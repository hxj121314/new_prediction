...
# db = KMeans(n_clusters=k,init='k-means++').fit(X)
skl_labels = db.labels_
frm_index = np.arange(stf+1,enf+1,1)
print(np.vstack((frm_index,skl_labels+((stf/500)*k))))
np.savetxt(path+'/result'+str(stf/500)+'.csv',np.vstack((frm_index,skl_labels+((stf/500)*k))).T,delimiter=',')